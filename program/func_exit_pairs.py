...
def manage_trade_exits(client):
  ...
  for position in open_positions_dict:
    ...
    pnl = float(pnl_1) + float(pnl_2)
    pnl_percent = float(pnl_1) / (float(position_size_m1)*float(position_start_price_m1))*100 + float(pnl_2)/(float(position_size_m2)*float(position_start_price_m2))*100
    ...
    if is_close:
        ...
        try:
           ...
           send_message_main(f"Bot closed the pair:\n\n{position_market_m1}:\nSide: {side_m1}, Size: {position_size_m1}, Price: {accept_price_m1}$  \n-- VS -- \n{position_market_m2}: \
                        \nSide: {side_m2}, Size: {position_size_m2}, Price: {accept_price_m2}$\n\nZ-Score: {z_score_current}\nPair PnL: {round(pnl,2)}$\nPair Realized PnL %: {round(pnl_percent,2)}%")
           send_message_berta(f"Pair was closed:\n\n{position_market_m1}:\nSide: {side_m1}, Price: {accept_price_m1}$  \n-- VS -- \n{position_market_m2}: \
                        \nSide: {side_m2}, Price: {accept_price_m2}$\n\nZ-Score: {z_score_current}\nPair Realized PnL %: {round(pnl_percent,2)}%")
